...
const Guess = (): JSX.Element => {
  ...
  const handleNextTerm = (): void => {
    ...
    if (currentTermIndex + 1 === terms.length) {
      ...
      setTimers((prevTimers) =>
        prevTimers.map((timer, index) => (index === currentTermIndex ? 20 : timer))
      );
    }
  };
        ...
        setTimers((prevTimers) =>
          prevTimers.map((timer, index) => {
            if (index === currentTermIndex && timer === 1) {
              ...
              if (timerSoundElement.current && timerSoundElement.current.paused) {
  ...
  return (
    <div className="bg-gray-900 min-h-screen">
      <Navbar gameStarted={gameStarted} handleLogout={handleLogout} username={username} restartGame={restartGame} />
          ...
          <div className="square-content text-center">
            {!gameOver && (
              <p className="text-3xl mb-8 text-white text-center pulsing-text">
                ...
                <span className="text-orange-600 font-black text-4xl border-orange-500 pb-2 font-bold">WORD</span>
              </p>
            )}
            {!gameStarted ? (
              ...
              <>
                {!gameOver ? (
                  <div className="flex flex-col items-center mb-4">
                    <p className="text-lg mt-x4 mb-3 text-white text-center font-bold">{currentTerm.question}</p>
                    <div className="border rounded py-1 px-4 mt-3 mb-1">
                      <div className="flex flex-wrap justify-center">
                        {shuffledTerm.split('').map((letter: string, index: number) => (
                          <p key={index} className="text-4xl text-white letter-spacing-2">
                            {letter}
                          </p>
                        ))}
                      </div>
                    ...
                    <p
                      className={`text-stacktrek mt-1 font-bold text-sm ${
                        timers[currentTermIndex] <= 5 && timers[currentTermIndex] > 0
                          ? 'text-red-500'
                    ...
                    <div className="flex items-center justify-center mt-5">
                      <div className="w-12 bg-gray-300 rounded-full">
                        <div className={`${scoreProgressClassName} text-xs leading-none py-1 text-center text-white rounded-full`}>
                          {scoreProgress}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-center mt-2">
                      {isPassingScore ? (
                        <p className={`text-sm ml-2 font-bold text-green-500`}>Passing. ✓ </p>
                      ) : (
                        <p className={`text-sm ml-2 font-bold ${scoreDisplayClassName}`}>Not Passing.</p>
                      )}
                    </div>
                  </div>
                ) : (
                    ...
                    {tryAgain ? (
                      <>
                        <p className="text-3xl mb-10 text-white text-center animation-bounce">
                          <span className="text-red-500 font-bold text-4xl">You Didn't Pass the Quiz!</span>
                        </p>
                        <p className="text-white text-center text-2xl font-bold mb-10">Final Score: {score}</p>
                        <p className="text-white text-center text-2xl font-bold">
                          Scoring: {scoringPercentage}%</p>
                        ...
                        <p className="text-red-500 text-center text-xl font-bold mt-10">Please Try Again!</p>
                        <button
                          onClick={restartGame}
                      ...
                      <>
                        <p className="text-3xl mb-10 text-white text-center animation-bounce ">
                          <span className="text-blue-600 font-bold text-4xl">Congratulations! </span>
                          <span className="text-orange-600 font-bold text-4xl">You Passed the Quiz!</span>
                        </p>
                        <p className="text-white text-center text-2xl font-bold mb-10">Final Score: {score}</p>
                        <p className="text-white text-center text-2xl font-bold">Scoring: {scoringPercentage}%</p>
                        ...
                        <p className="text-orange-500 text-center text-xl font-bold mt-10">You're awesome!</p>
                        <button
                          onClick={restartGame}
